...
- `credential_renewal/reporting_export.py`: Log Analytics reporting export entry point.
...
- `grafana/credential-renewal-cases-dashboard.json`: Grafana dashboard template.
...
rg -n "^(<<<<<<<|=======|>>>>>>>)" README.md DETAILED_EXPLANATION_AND_DEPLOYMENT.md DEPLOYMENT.md agents.md grafana/credential-renewal-cases-dashboard.json
```
